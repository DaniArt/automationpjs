...
def CheckAddedFullProfile_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['USER_ACC']:
                name_file = f'{card.GetRandomNumber()}_CheckAddedFullProfile.json'   
                CheckAddedFullProfile(row['CLI_ID'], row['TARIF_CODE'], row['USER_ACC'], name_file)
...
def CheckAddedFullProfile(cli_id, tarif, cliacc, name_file):
    card = TaskMethods()
    try:
        ...
        Delay(3000)
        # TO DO
        Sys.Process("COLVIR").VCLObject("InformDeaDtl").VCLObject("pnlClient").VCLObject("pnlSupport").VCLObject("pnlBottom").VCLObject("tabDetail").VCLObject("tsLinkedProfiles").VCLObject("grLnkPrf").TextObject("Платная СМС рассылка на рус.яз. (Полный шаблон)").Click()
#        card.ClickInputField("InformDeaDtl", 'TextObject("Платная СМС рассылка на рус.яз. (Полный шаблон)")', need_tab = True)
        DocName = card.GetGridDataFields("InformDeaDtl", "LONGNAME")
        CodeSc = DocName[0].replace("\'", '')
        card.CheckCorrectData(CodeSc, 'ПлатнаяСМСрассылканарус.яз.(Полный шаблон)', 1, name_file, 'VCLObject("InformDeaDtl")')
