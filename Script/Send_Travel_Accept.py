...
def SendTravelAccept_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['ITER_CODE'] == '1':
                name_file = f'{NCRD.GetRandomNumber()}_SendTravelAccept.json'  
                form_data = SendTravelAccept(row['CLI_ID'], name_file)
                row['ACC_CRD'] = form_data
                NCRD.UpdateDatasetTableDB('travel_data', data_list)
            elif row['IDN_CARD'] is None:
                name_file = f'{NCRD.GetRandomNumber()}_SendTravelAccept.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Отправка на акцептование карты Travel", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
          
            
def SendTravelAccept(cli_id, name_file):
