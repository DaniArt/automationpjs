...
def IronCashout2401_AllDataSet():
   ...
   for row in data_list:
       try:
           if row['IDN_CARD']:
               name_file = f'{card.GetRandomNumber()}_IronCashout2401.json'
               IronCashout2401(row['IDN_CARD'], row['ACC_CRD'], row['SUM_TRN'], row['VAL_CODE'], name_file)
           elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_IronCashout2401.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие со счета через операцию 2401 Iron карта клиента {row['CLI_ID']}",
...
def IronCashout2401(card_idn, acc_crd, amount, val, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Iron карта", f"Снятие со счета через операцию 2401 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
