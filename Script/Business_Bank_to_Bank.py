...
def BusinessBanktoBank_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_BusinessBanktoBank.json' 
                BusinessBanktoBank(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['SUM_TRN'], row['CARDCODE'], name_file)  
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_BusinessBanktoBank.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Межбанковский перевод 2560 Business карта клиента {row['CLI_ID']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                              
                                                                                                            
                                                                                                            
def BusinessBanktoBank(valcode, acc_crd, idn_crd, amount, card_num, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Business карта", f"Межбанковский перевод 2560 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
