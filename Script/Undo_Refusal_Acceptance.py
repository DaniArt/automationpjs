...
def UndoRefusalAcceptance_AllDataSet():
    """ Отмена отказа в акцепте карты клиента по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
...
def UndoRefusalAcceptance(idn_card, name_file):
    """ Отмена отказа в акцепте карты клиента """
    NCRD = CardFramework() 
    ...
    value = ["Акцепт карты - Отмена отказа в акцепте", f"Отмена отказа в акцепте карты {idn_card}", NCRD.GetDateTimeMilli()]
