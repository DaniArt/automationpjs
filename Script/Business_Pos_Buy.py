...
def BusinessPosBuy_AllDataSet():
    """ Отправка запроса по покупке через POS2416 """
    NCRD = CardFramework()
    data_list = NCRD.ReadDatasetFromDB('ndeacrd_data')
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{NCRD.GetRandomNumber()}_BusinessPosBuy.json'
                BusinessPosBuy(row['IDN_CRD'], row['CLI_ACC'], row['AMOUNT'],
                              row['VAL'], row['CARD_NUM'], row['PSYS'], row['TERM_TYPE'],
                              row['MCC'], row['TYPE_TRN'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{NCRD.GetRandomNumber()}_BusinessPosBuy.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Проверка бонусов через POS покупку Business карта клиента {row['CLI_ID']}",
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
  
def BusinessPosBuy(idn_crd, acc_crd, amount, val, card_num, psys, term_type, mcc, type_trn, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Business карта", f"Проверка бонусов через POS покупку по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
