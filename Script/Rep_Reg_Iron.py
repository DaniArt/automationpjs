...
def RepRegisterIronCKC_AllDataSet():
    ...
    for row in data_list:  
        try:
            if row['DOC_CODE']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RepRegisterIron.json'  
                RegisterIronCKC(row['DOC_CODE'],name_file)
            elif row['DOC_CODE'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RepRegisterIron.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Повторная регистрация договора СКС Iron карта", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def RegisterIronCKC(DOC_CODE,name_file):
    ...
    value = ["Повторная регистрация договора СКС", "Повторная регистрация договора СКС для клиента", Ndeacrd.GetDateTimeMilli()]
