...
def TravelCashout2411_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_TravelCashout2411.json'
                TravelCashout2411(row['CLI_ACC'], row['IDN_CRD'], row['AMOUNT'],
                               row['CARD_NUM'], row['PSYS'], row['TERM_TYPE'],
                               row['MCC'], row['TYPE_TRN'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelCashout2411.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие со счета карты через операцию 2411 Travel карта клиента {row['CLI_ID']}",
...
def TravelCashout2411(acc_crd, idn_crd, amount, card_num, psys, term_id, mcc, type_trn, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Снятие со счета карты через операцию 2411 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
