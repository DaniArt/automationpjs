...
def UndoRegisterCKC_AllDataSet():
    ...
    for row in data_list:  
        try:
            if row['DOC_CODE'] and row['DEPO_CODE'] == 'AA6':
                name_file = f'{Ndeacrd.GetRandomNumber()}_UndoRegisterCKC.json'  
                UndoRegisterCKC(row['DOC_CODE'],name_file)
            elif row['DOC_CODE'] is None and row['DEPO_CODE'] == 'AA6':
                name_file = f'{Ndeacrd.GetRandomNumber()}_RegistrDocCKC.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Отмена регистрации договора СКС", 
                                                    f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def UndoRegisterCKC(DOC_CODE,name_file):
