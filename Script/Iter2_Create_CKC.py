...
def Iter2CreateCKC_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{Ndeacrd.GetRandomNumber()}_Iter2CreateCKC.json'   
            form_data = Iter2CreateCKC(row['DEPO_CODE'],row['VAL_CODE'],
                                       row['CLI_ID'],row['TARIF_CODE'], 
                                       row['TRF_DOG'],name_file)
            row['DOC_CODE'] = form_data
            Ndeacrd.UpdateDatasetTableDB('ckcdoc_data', data_list)

def Iter2CreateCKC(depo_code,val_code,cli_id,tarif_code,ckc_code,name_file):
