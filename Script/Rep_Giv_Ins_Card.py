...
def RepGivInsCard_AllDataSet():
    """Повторная подвязка карты моментального выпуска к договору СКС по всему датасету """
    ...
    for row in data_list:
        try:
            if row['DOC_CODE'] and row['DEPO_CODE'] == 'AA6':
                ...
                name_file = f'{Ndeacrd.GetRandomNumber()}_GiveoutInstantCard.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Повторная подвязка инстант карты к договору СКС после отмены подвязки", 
...
def GiveoutInstantCard(product,cl_code,doc_code,reg_date, name_file):
    ...
    value = ["Повторная подвязка инстант карты к договору СКС после отмены подвязки", f"Повторная подвязка карты {product} к договору {doc_code}", Ndeacrd.GetDateTimeMilli()]
    ...
    try:
      ...
      if form_data[1].replace("\'", "") == 'Заявлениенавыпусквведено': # Проверка состояния документа
        # Отчетность
        Ndeacrd.AllureReportTemplate(abs_path, name_file, "Повторная подвязка договора СКС для клиента", "passed", {"message": f"Карта с идентификатором {form_data[0]} в состоянии {form_data[1]}"},
        ...
        # Отчетность
        Ndeacrd.AllureReportTemplate(abs_path, name_file, "Повторная подвязка договора СКС для клиента", "failed", {"message": f"Операция {form_data[1]} по карте {form_data[0]} не выполнена"},
