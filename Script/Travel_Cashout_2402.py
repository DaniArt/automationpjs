    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                ...
                name_file = f'{card.GetRandomNumber()}_TravelCashout2402.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие со счета карты через операцию 2402 АТМ Travel карта клиента {row['CLI_ID']}",
...
def TravelCashout2402(val, acc_crd, idn_crd, amount, name_file):
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Снятие через операцию 2402 АТМ по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
        ...
        response = soap.ATM_2402(idn_crd, amount, val, 'ON-US')
