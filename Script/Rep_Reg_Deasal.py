...
def RepRegisterDeasalDog_AllDataSet():
    ...
    for row in data_list:
        try:  
            if row['PRJ_CODE'] and row['DEASAL']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RepRegisterDeasalDog.json'  
                RegisterDeasalDog(row['CLI_ID'], row['PRJ_CODE'],name_file)
            elif row['PRJ_CODE'] is None and row['DEASAL'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RepRegisterDeasalDog.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Повторная регистрация зарплатного проекта DEASAL", 
...
def RegisterDeasalDog(cli_code, prj_code, name_file):
    ...
    value = ["Повторная регистрация зарплатного проекта DEASAL", f"Повторная регистрация  зарплатного проекта {prj_code}", Ndeacrd.GetDateTimeMilli()]
    ...
    try:
        ...
        if status_doc[0].replace("\'", '') == 'Зарегистрирован':
            # Отчетность
            Ndeacrd.AllureReportTemplate(abs_path, name_file, "Повторная регистрация  зарплатного проекта", "passed", {"message": f"Документ с номером {status_doc[1]} в состоянии {status_doc[0]}"},
            ...
            # Отчетность
            Ndeacrd.AllureReportTemplate(abs_path, name_file, "Повторная регистрация  зарплатного проекта", "failed", {"message": f"Документ с номером {status_doc[1]} в состоянии {status_doc[0]}"},
