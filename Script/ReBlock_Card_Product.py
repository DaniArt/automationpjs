...
def ReBlockCardProduct_AllDataSet():
    """ Обратная блокировка пластиковой карты клиента по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_ReBlockCardProduct.json'   
            ReBlockCardProduct(row['IDN_CARD'], name_file)

            
def ReBlockCardProduct(idn_card, name_file):
    """ Обратная блокировка пластиковой карты клиента """
    NCRD = CardFramework() 
    ...
    value = ["Обратная блокировка пластиковой карты", f"Обратная блокировка пластиковой карты {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:      
        ...
        if status_crd[0].replace("\'", '') == '41':
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Выполнение операции блокировки карты", "passed", {"message": f"Карта с идентификатором {status_crd[1]} заблокирована"},
                                         'VCLObject("NCrdList")', "Карта заблокирована", new_path, "passed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Checkpoint(f'Карта с идентификатором {status_crd[1]} заблокирована')
        else:
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Выполнение операции блокировки карты", "failed", {"message": f"Карта с идентификатором {status_crd[1]} активирована"},
                                         'VCLObject("NCrdList")', "Карта не заблокирована", new_path, "failed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Warning(f'Карта с идентификатором {status_crd[1]} активирована')
