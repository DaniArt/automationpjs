...
def Iter2RegisterCKC_AllDataSet():
    ...
    for row in data_list:  
        if row['ITER_CODE'] == '2':
            name_file = f'{Ndeacrd.GetRandomNumber()}_Iter2RegisterCKC.json'  
            form_data = Iter2RegisterCKC(row['DOC_CODE'],name_file)
            row['ACC_CRD'] = form_data
            Ndeacrd.UpdateDatasetTableDB('ckcdoc_data', data_list)
            
def Iter2RegisterCKC(DOC_CODE,name_file):
