...
def CreatePullForNdeacrd_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{Card.GetRandomNumber()}_CreatePullForNdeacrd.json'   
            form_data = CreatePullForNdeacrd(row['DEPO_CODE'],row['PRODUCT'],row['VAL_CODE'], row['AMOUNT'], name_file)
            row['REG_DATE'] = form_data
            Card.UpdateDatasetTableDB('ckcdoc_data', data_list)

def CreatePullForNdeacrd(depo_code,product,val_code,amount,name_file):
