...
def TravelCashin2543_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_TravelCashin2543.json' 
                TravelCashin2543(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['SUM_TRN'], name_file)  
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelCashin2543.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Пополнение счета по операции 2543 Travel карта клиента {row['CLI_ID']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                              
                                                                                                            
                                                                                                            
def TravelCashin2543(valcode, acc_crd, idn_crd, amount, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Пополнение счета по операции 2543 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
