...
def Iter2SendToAccept_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{NCRD.GetRandomNumber()}_Iter2SendToAccept.json'  
            form_data = Iter2SendToAccept(row['IDN_CARD'], name_file)
            row['ACC_CRD'] = form_data
            NCRD.UpdateDatasetTableDB('ckcdoc_data', data_list)
            
def Iter2SendToAccept(idn_card, name_file):
