...
def RegisterTravelCKC_AllDataSet():
    ...
    for row in data_list:  
        try:
            if row['DOC_CODE']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RegisterTravelCKC.json'  
                RegisterTravelCKC(row['DOC_CODE'],name_file)
            elif row['DOC_CODE'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RegisterTravelCKC.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Регистрация договора СКС Travel", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def RegisterTravelCKC(DOC_CODE,name_file):
