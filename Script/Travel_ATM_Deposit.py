...
def TravelATMDeposit_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_TravelATMDeposit.json' 
                TravelATMDeposit(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['SUM_TRN'], row['CARDCODE'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelATMDeposit.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Пополнение счета карты Travel карта клиента {row['CLI_ID']}", 
...
def TravelATMDeposit(valcode, acc_crd, idn_crd, amount, card_num, name_file):
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Пополнение счета через операцию 2405 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
        ...
        response = soap.ATM2405(acc_crd, amount, valcode)
