...
def SendToStopList_AllDataSet():
    """ Операция постановки в стоп-лист"""
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_SendToStopList.json'   
            SendToStopList(row['IDN_CARD'], name_file)

            
def SendToStopList(idn_card, name_file):
    """ Операция постановки в стоп-лист """
    NCRD = CardFramework() 
    ...
    value = ["Постановка в стоп-лист", f"Операция постановка в стоп-лист по {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:      
        ...
        NCRD.FindNeedOperation("Постановка в стоп-лист") 
        NCRD.ConfirmOperation("Постановка в стоп-лист")
        ...
        if status_crd[0].replace("\'", '') == 'Постановкавстоп-лист':
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Постановка в стоп-лист", "passed", {"message": f"Карта с идентификатором {status_crd[1]} поставлена в стоп-лист"},
                                         'VCLObject("frmOperJournal")', "Выполнен", new_path, "passed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Checkpoint(f'Карта с идентификатором {status_crd[1]} поставлена в стоп-лист')
        else:
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Постановка в стоп-лист", "failed", {"message": f"Карта с идентификатором {status_crd[1]} не поставлена"},
                                         'VCLObject("frmOperJournal")', "Ошибка", new_path, "failed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Warning(f'Карта с идентификатором {status_crd[1]} не поставлена')
