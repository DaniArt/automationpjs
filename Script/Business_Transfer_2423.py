    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                ...
                BusinessTransfer2423(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['AMOUNT'], row['CARDCODE'], name_file)  
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_BusinessTransfer2423.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Переводы P2P 2423 Card Debit Business карта клиента {row['CLI_ID']}", 
...
def BusinessTransfer2423(valcode, acc_crd, idn_crd, amount, card_num, name_file):
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Business карта", f"Переводы P2P 2423 Card Debit по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
