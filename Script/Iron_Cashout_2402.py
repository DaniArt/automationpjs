...
def IronCashout2402_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_IronCashout2402.json'
                IronCashout2402(row['ACC_CRD'], row['IDN_CARD'], row['SUM_TRN'], row['VAL_CODE'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_IronCashout2402.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие через операцию 2402 чужой АТМ Iron карта клиента {row['CLI_ID']}",
...
def IronCashout2402(acc_crd, idn_crd, amount, val, name_file):
    soap = SoapRequests() 
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Iron карта", f"Снятие через операцию 2402 чужой АТМ по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
