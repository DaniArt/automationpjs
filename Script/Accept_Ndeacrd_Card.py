...
def AcceptNdeacrdCard_AllDataSet():
    """ Принятие решения об акцепте карты клиента по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_AcceptNdeacrdCard.json'   
            form_data = AcceptNdeacrdCard(row['IDN_CARD'], name_file)
            cardcode, bon_code = form_data
            row['CARDCODE'] = cardcode
            row['BON_CODE'] = bon_code
            NCRD.UpdateDatasetTableDB('ckcdoc_data', data_list)
            
def AcceptNdeacrdCard(idn_card, name_file):
    """ Принятие решения об акцепте карты клиента """
    cardcode = ''
    bon_code = ''
    NCRD = CardFramework() 
    ...
    try:      
        ...
        form_data = status_crd[3].replace("\'", '')
        cardcode = form_data
