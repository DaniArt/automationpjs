...
def ChangeDepartmentCKC_AllDataSet():
    """ Перевод в другое подразделение договора СКС для клиента по всему датасету """
    Ndeacrd = CardFramework()
    Ndeacrd.LoginInColvir()
    data_list = Ndeacrd.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:  
        if row['ITER_CODE'] == '1':
            name_file = f'{Ndeacrd.GetRandomNumber()}_ChangeDepartmentCKC.json'  
            ChangeDepartmentCKC(row['DOC_CODE'], row['CLI_ID'], row['ACC_CRD'], name_file)
            
def ChangeDepartmentCKC(DOC_CODE, cli_code, acc_code, name_file):
    """ Перевод в другое подразделение договора СКС для клиента """
    Ndeacrd = CardFramework()
    ...
    value = ["Перевод в другое подразделение", f"Операция Перевода в другое подразделение по договору {DOC_CODE}", Ndeacrd.GetDateTimeMilli()]
    ...
    try:  
        ...
        Ndeacrd.FindNeedOperation("Перевод в другое подразделение") 
        Ndeacrd.ConfirmOperation("Перевод в другое подразделение")
        Ndeacrd.WaitLoadWindow('frmDynamicDialog', time_await=3000)
        depo_now = Ndeacrd.FindChildField("frmDynamicDialog", "Name", "VCLObject('SRV_DEP_CODE')")
        depo_now.Keys('AE2')
        center_cash = Ndeacrd.FindChildField("frmDynamicDialog", "Name", "VCLObject('SELL_DEP_CODE')")
