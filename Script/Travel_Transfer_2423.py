...
def TravelTransfer2423_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_TravelTransfer2423.json' 
                TravelTransfer2423(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['AMOUNT'], row['CARDCODE'], name_file)    
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelTransfer2423.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Переводы P2P 2423 Travel карта клиента {row['CLI_ID']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                           
                                                                                                            
                                                                                                            
def TravelTransfer2423(valcode, acc_crd, idn_crd, amount, card_num, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Переводы P2P 2423 Card Debit по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
        ...
        task.AllureReportEnd(count, name_file, "passed")
        count += 1
        #---------
