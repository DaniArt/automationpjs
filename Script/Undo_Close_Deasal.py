...
def UndoCloseDeasal_AllDataSet():
    """ Отмена закрытия зарплатного проекта по всему датасету """
    ...
    for row in data_list:  
        try:
            if row['PRJ_CODE'] and row['DEASAL']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_UndoCloseDeasal.json'  
                UndoCloseDeasal(row['CLI_ID'], row['PRJ_CODE'],name_file)
            elif row['PRJ_CODE'] is None and row['DEASAL'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_CloseDeasalDog.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Отмена закрытия зарплатного проекта", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
                
            
def UndoCloseDeasal(cli_code, prj_code, name_file):
    """ Отмена закрытия зарплатного проекта """
    Ndeacrd = CardFramework()
    count = 1
    ...
    value = ["Отмена закрытия зарплатного проекта", f"Отмена закрытия зарплатного проекта {prj_code}", Ndeacrd.GetDateTimeMilli()]
    ...
    try:
        ...
        Ndeacrd.SetFilter(CLI_CODE=cli_code) # Вставляем номер документа из бд, для поиска нашего заведенного договора
        Delay(30000)
        ...
        Ndeacrd.FindNeedOperation("Отмена закрытия") 
        Ndeacrd.ConfirmOperation("Отмена закрытия")
        Delay(2000)
        ...
        if status_doc[0].replace("\'", '') == 'Зарегистрирован':
            # Отчетность
            Ndeacrd.AllureReportTemplate(abs_path, name_file, "Отмена закрытия зарплатного проекта", "passed", {"message": f"Документ с номером {status_doc[1]} в состоянии {status_doc[0]}"},
            ...
            Ndeacrd.AllureReportTemplate(abs_path, name_file, "Закрытие зарплатного проекта", "failed", {"message": f"Документ с номером {status_doc[1]} в состоянии {status_doc[0]}"},
                                           'VCLObject("frmaNDeaSalLst")', "Отмена закрцтия не выполнена", new_path, "failed", 1, 1, rm=True)
            # --------------------------------------------------------------------------------------------------------------------------
