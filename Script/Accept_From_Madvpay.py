...
def AcceptFromMadvpay_AllDataSet():
    """ Оприходывание карты через Madvpay по всему датасету """
    Madvpay = CardFramework()
    login_user, pass_user = Madvpay.GetLoginPass('BOC_OFICER')
    Madvpay.LoginInColvir(login_user, pass_user)
    data_list = Madvpay.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:  
        if row['DEPO_CODE'] == 'AA6':
            name_file = f'{Madvpay.GetRandomNumber()}_AcceptFromMadvpay.json'  
            AcceptFromMadvpay(row['DOC_CODE'], row['CLI_ID'], row['ACC_CRD'], name_file)
            
def AcceptFromMadvpay(DOC_CODE, cli_code, acc_code, name_file):
    ...
    try:  
        ...
        for i in range(10):
            if MadvpayGrid[0].replace("\'", '') == 'Передан':
                ...
                Madvpay.FindNeedOperation("Принять") 
