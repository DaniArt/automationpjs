...
def SendAcceptCardNdeacrd_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_SendAcceptCardNdeacrd.json'  
            form_data = SendAcceptCardNdeacrd(row['IDN_CARD'], name_file)
            row['ACC_CRD'] = form_data
            NCRD.UpdateDatasetTableDB('ckcdoc_data', data_list)
            
def SendAcceptCardNdeacrd(idn_card, name_file):
