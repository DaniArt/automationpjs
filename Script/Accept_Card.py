    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['DEPO_CODE'] == 'AA6':
                ...
                form_data = AcceptCard(row['CLI_ID'],row['IDN_CARD'],name_file)
                ...
                name_file = f'{NCRD.GetRandomNumber()}_AcceptCard.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Принять решение по акцепту", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def AcceptCard(cl_code,idn_card,name_file):
    ...
    value = ["Принять решение по акцепту", f"Принятие решения по акцептованию карты {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:            
        ...
        NCRD.SetFilter(CLI_code=cl_code,CARDIDN=idn_card) # Берем номер идентификатора из бд, для поиска нашей карты
        ...
        btn_ok.Click()
        if not NCRD.ErrorMessageHandler(recursive='True'):
            ...
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Выполнение операции акцептования карты", "failed", {"message": f"Операция не выполнена"},
                                         'VCLObject("NCrdList")', "Карта не акцептована", new_path, "failed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Warning(f'Операция не выполнена')
            count += 1 
