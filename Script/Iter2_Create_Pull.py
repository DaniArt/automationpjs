...
def Iter2CreatePull_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{Card.GetRandomNumber()}_Iter2CreatePull.json'   
            form_data = Iter2CreatePull(row['DEPO_CODE'],row['PRODUCT'],row['VAL_CODE'], row['AMOUNT'], name_file)
            row['REG_DATE'] = form_data
            Card.UpdateDatasetTableDB('ckcdoc_data', data_list)

def Iter2CreatePull(depo_code,product,val_code,amount,name_file):
