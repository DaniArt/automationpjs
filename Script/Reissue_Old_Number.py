...
def ReissueOldNumber_AllDataSet():
    """ Перевыпуск со старым номером """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{NCRD.GetRandomNumber()}_ReissueOldNumber.json'   
            ReissueOldNumber(row['IDN_CARD'], name_file)

            
def ReissueOldNumber(idn_card, name_file):
    NCRD = CardFramework() 
    ...
    value = ["Перевыпуск карты со старым номером", f"Перевыпуск со старым номером {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:      
        ...
        NCRD.FindNeedOperation("Блокировать карту") 
        NCRD.ConfirmOperation("Блокировать карту")
        ...
        describe.Keys('Перевыпуск со старым номером')
        ...
        NCRD.WaitLoadWindow('NCrdList')
        but_operations.Click()
        NCRD.FindNeedOperation("Перевыпуск со старым номером") 
        NCRD.ConfirmOperation("Перевыпуск со старым номером")
        if Sys.Process("COLVIR").WaitVCLObject("frmDynLstRefer", 5000).Exists:
            ok_btn = NCRD.FindChildField("frmDynLstRefer", "Name", "VCLObject('btnOK')")
            ok_btn.Click()
            NCRD.CheckOperEndWindow()
        else:
            Log.Event("Объект frmDynLstRefer не найден, продолжаем выполнение теста...")
        NCRD.CheckOperEndWindow()
        NCRD.WaitLoadWindow('NCrdList')
        ...
        if status_crd[0].replace("\'", '') == '41':
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Перевыпуск карты со старым номером", "passed", {"message": f"Карта с идентификатором {status_crd[1]} перевыпущена"},
                                         'VCLObject("NCrdList")', "Карта заблокирована", new_path, "passed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Checkpoint(f'Карта с идентификатором {status_crd[1]} перевыпущена')
        else:
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Перевыпуск карты со старым номером", "failed", {"message": f"По карте {status_crd[1]} не выполнен перевыпуск"},
                                         'VCLObject("NCrdList")', "Карта не заблокирована", new_path, "failed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Warning(f'По карте {status_crd[1]} не выполнен перевыпуск')
