    ...
    for row in data_list:  
        try:
            if row['DOC_CODE'] and row['DEPO_CODE'] == 'AA6':
              ...
              name_file = f'{Ndeacrd.GetRandomNumber()}_RegistrDocCKC.json'
              Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Регистрация договора - Регистрация договора СКС для клиента", 
...
def RegistrDocCKC(DOC_CODE,name_file):
    ...
    value = ["Регистрация договора - Регистрация договора СКС для клиента", f"Регистрация договора {DOC_CODE} СКС", Ndeacrd.GetDateTimeMilli()]
