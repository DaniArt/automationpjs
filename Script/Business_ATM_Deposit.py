...
def BusinessATMDeposit_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_BusinessATMDeposit.json'
                BusinessATMDeposit(row['VAL_CODE'], row['SUM_IN'], row['ACC_CRD'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_BusinessATMDeposit.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Пополнение счета через операцию 2405 Business карта клиента {row['CLI_ID']}",
...
def BusinessATMDeposit(val, sum_in, acc_crd, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Business карта", f"Пополнение счета через операцию 2405 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
