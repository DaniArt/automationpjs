...
def CreateBusinessCard_AllDataSet():
    ...
    for row in data_list:
        try:
          if row['DOC_CODE'] and row['ITER_CODE'] == '1':
              name_file = f'{Ndeacrd.GetRandomNumber()}_CreateBusinessCard.json' 
              form_data = CreateBusinessCard(row['PRODUCT'],row['CLI_ID'], row['DOC_CODE'], row['DEPO_CODE'], row['TARIF_CODE'], name_file)
              row['IDN_CARD'] = form_data
              Ndeacrd.UpdateDatasetTableDB('ndeacrd_data', data_list)
          elif row['DOC_CODE'] is None and row['ITER_CODE'] == '1':
                name_file = f'{Ndeacrd.GetRandomNumber()}_CreateBusinessCard.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Подвязка карты моментального выпуска к договору СКС", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
              
def CreateBusinessCard(product,cl_code, doc_code, depo, tarif, name_file):
    ...
    try:
        ...
        ok_choose3.Click()
        Ndeacrd.WaitLoadWindow('NCardDetail', time_await=3000)
        ...
        code_word.Click()
        if Sys.Process("COLVIR").WaitDialog("Colvir Banking System", 3000).Exists:
            Sys.Process("COLVIR").Dialog("Colvir Banking System").UIAObject("Colvir_Banking_System").Window("CtrlNotifySink", "", 7).Button("OK").Click()
            emboss_name.Keys('TestName')
            code_word.Click()
