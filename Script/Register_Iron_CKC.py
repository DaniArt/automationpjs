...
def RegisterIronCKC_AllDataSet():
    ...
    for row in data_list:  
        try:
            if row['DOC_CODE']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RegisterIronCKC.json'  
                RegisterIronCKC(row['DOC_CODE'],name_file)
            elif row['DOC_CODE'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_RegisterIronCKC.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Регистрация договора СКС Iron карта", 
