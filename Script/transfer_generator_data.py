...
def TestDataPreparationForArestFull_AllDataSet():
  ...
  data_list = PaydocDataset.ReadDatasetFromDB('cliacc_cashback')
  for row in data_list:
    if row['TYPE_CLI'] == 'FL':
        get_new_iin, get_new_acc, get_code_cli = TestDataPreparationForCheckLimit(row['VAL'], row['TYPE_CLI'])
        row['IIN_BIN'] = get_new_iin 
        row['cli_acc'] = get_new_acc
        row['CLI_ID'] = get_code_cli
        PaydocDataset.UpdateDatasetTableDB('cliacc_cashback', data_list)
