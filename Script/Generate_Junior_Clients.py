...
def GenerateJuniorClients_AllDataSet():
  ...
  data_list = PaydocDataset.ReadDatasetFromDB('junior_data')
  for row in data_list:
    if row['ITER_CODE'] == '1':
        ...
        PaydocDataset.UpdateDatasetTableDB('junior_data', data_list)
...
def CreateCliCard(val_code,cli_type,get_new_iin,sex=None,res=None,birthday=None,dateReg=None,
  ...
  if cli_type == 'FL':  
    codeCli = Paydoc.OpenNewCardMclienBoks(get_new_iin,res,type_doc,'CLI',cli_type,name,surName,nameCom,sex,full_name,'CNT',birthday,dateReg)
