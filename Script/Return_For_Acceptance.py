...
def ReturnForAcceptance_AllDataSet():
    """ Возвращение обратно карты на акцепт по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{NCRD.GetRandomNumber()}_ReturnForAcceptance.json'   
            ReturnForAcceptance(row['IDN_CARD'], name_file)
            
def ReturnForAcceptance(idn_card, name_file):
    """ Возвращение обратно карты на акцепт """
    NCRD = CardFramework() 
    ...
    value = ["Акцепт карты - Вернуть на акцептование", f"Вернуть на акцептование карты {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:      
        ...
        NCRD.FindNeedOperation("Вернуть на акцептование") 
        NCRD.ConfirmOperation("Вернуть на акцептование")
        ...
        if status_crd[0].replace("\'", '') == 'Переданонаакцептование':
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Вернуть на акцептование", "passed", {"message": f"Карта с идентификатором {status_crd[1]} в состоянии {status_crd[0]}"},
            ...
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Вернуть на акцептование", "failed", {"message": f"Карта с идентификатором {status_crd[1]} в состоянии {status_crd[0]}"},
