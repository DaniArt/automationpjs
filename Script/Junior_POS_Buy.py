...
def JuniorPosBuy_AllDataSet():
    """ Отправка запроса по покупке через POS2416 """
    NCRD = CardFramework()
    data_list = NCRD.ReadDatasetFromDB('junior_data')
    for row in data_list:
      try:
          if row['IDN_CARD']:
              name_file = f'{NCRD.GetRandomNumber()}_JuniorPosBuy.json'   
              JuniorPosBuy(row['IDN_CRD'], row['CLI_ACC'], row['AMOUNT'], 
                ...
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Проверка бонусов через POS покупку",
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
      except Exception as e:
          Log.Event('Возникла ошибка идем дальше')
  
def JuniorPosBuy(idn_crd, acc_crd, amount, val, card_num, psys, term_type, mcc, type_trn, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Junior карта", "Проверка бонусов через POS покупку", name_file)
        #---------    
        soap.StartCapJob()
