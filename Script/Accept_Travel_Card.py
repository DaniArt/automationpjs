...
def AcceptTravelCard_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['ITER_CODE'] == '1':
                name_file = f'{NCRD.GetRandomNumber()}_AcceptTravelCard.json'   
                form_data = AcceptTravelCard(row['IDN_CARD'], name_file)
                cardcode, bon_code = form_data
                row['CARDCODE'] = cardcode
                row['BON_CODE'] = bon_code
                NCRD.UpdateDatasetTableDB('travel_data', data_list)
            elif row['IDN_CARD'] is None:
                name_file = f'{NCRD.GetRandomNumber()}_AcceptTravelCard.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Принятие решения по акцепту Travel", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def AcceptTravelCard(idn_card, name_file):
    ...
    value = ["Принятие решения по акцепту", f"Принятие решения по акцептованию карты {idn_card}", NCRD.GetDateTimeMilli()]
