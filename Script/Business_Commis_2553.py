...
def BusinessCommis2553_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_BusinessCommis2553.json' 
                BusinessCommis2553(row['VAL_CODE'], row['ACC_CRD'], row['SUM_TRN'], row['CARDCODE'], name_file)
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_BusinessCashout2411.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Сервисная комиссия 2553 по счету {row['ACC_CRD']}",
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                         
                                                                                                            
                                                                                                            
def BusinessCommis2553(valcode, acc_crd, amount, card_num, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Business карта", f"Сервисная комиссия 2553 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
        ...
        response = soap.ServiceCommis(acc_crd, valcode, amount)
