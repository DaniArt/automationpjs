...
def CreateTravelCKC_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['ITER_CODE'] == '1':
                name_file = f'{Ndeacrd.GetRandomNumber()}_CreateTravelCKC.json'   
                form_data = CreateTravelCKC(row['DEPO_CODE'],row['VAL_CODE'],
...
def CreateTravelCKC(depo_code,val_code,cli_id,tarif_code,ckc_code,name_file):
