    ...
    for row in data_list:
        try:
            if row['DOC_CODE']:
                ...
                name_file = f'{Ndeacrd.GetRandomNumber()}_CreateIronCard.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Подвязка карты Iron к договору СКС", 
...
def CreateIronCard(product, cli_id, depo, tarif, name_file):
    ...
    value = ["Подвязка договора СКС", "Подвязка карты Iron к договору СКС", Ndeacrd.GetDateTimeMilli()]
