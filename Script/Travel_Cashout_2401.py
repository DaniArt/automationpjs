...
def TravelCashout2401_AllDataSet():
   ...
   for row in data_list:
       try:
           if row['IDN_CARD']:
               name_file = f'{card.GetRandomNumber()}_TravelCashout2401.json'
               TravelCashout2401(row['VAL_CODE'], row['IDN_CARD'], row['ACC_CRD'], row['SUM_TRN'], name_file)
           elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelCashout2401.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие со счета карты через операцию 2401 Travel карта клиента {row['CLI_ID']}",
...
def TravelCashout2401(val, card_idn, acc_crd, amount, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Снятие со счета через операцию 2401 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
