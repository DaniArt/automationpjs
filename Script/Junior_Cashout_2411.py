...
def JuniorCashout2411_AllDataSet():
    ...
    for row in data_list:
      try:
          if row['IDN_CARD']:
                ...
                JuniorCashout2411(row['CLI_ACC'], row['IDN_CRD'], row['AMOUNT'], 
                               row['CARD_NUM'], row['PSYS'], row['TERM_TYPE'], 
                               row['MCC'], row['TYPE_TRN'], name_file)
          elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_JuniorCashout2411.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Снятие со счета карты через операцию 2411 по клиенту {row['CLI_ID']}",
...
def JuniorCashout2411(acc_crd, idn_crd, amount, card_num, psys, term_id, mcc, type_trn, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Junior карта", f"Снятие со счета карты через операцию 2411 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
