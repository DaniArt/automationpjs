...
def IronExtract2451_AllDataSet():
    """ Мини выписка 2451 """
    card = SoapRequests()
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_IronExtract2451.json' 
                IronExtract2451(row['VAL_CODE'], row['ACC_CRD'], row['IDN_CARD'], row['SUM_TRN'], name_file)    
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_IronExtract2451.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Мини выписка 2451 Iron карта клиента {row['CLI_ID']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                         
                                                                                                            
                                                                                                            
def IronExtract2451(valcode, acc_crd, idn_crd, amount, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Iron карта", f"Мини выписка 2451 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
