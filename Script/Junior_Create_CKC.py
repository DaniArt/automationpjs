...
def JuniorCreateCKC_AllDataSet():
    """ Создание документа СКС для клиента по всему датасету """
    Ndeacrd = CardFramework()
    Ndeacrd.LoginInColvir()
    data_list = Ndeacrd.ReadDatasetFromDB('junior_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{Ndeacrd.GetRandomNumber()}_JuniorCreateCKC.json'   
            form_data = JuniorCreateCKC(row['DEPO_CODE'],row['VAL_CODE'],
                                       row['CLI_ID'],row['TRF_DOG'], 
                                       row['CKC_CODE'],name_file)
            row['DOC_CODE'] = form_data
            Ndeacrd.UpdateDatasetTableDB('junior_data', data_list)

def JuniorCreateCKC(depo_code,val_code,cli_id,tarif_code,ckc_code,name_file):
    ...
    value_labels_subsuite = ["subSuite", "Продукт - Junior карта"]
