...
def AcceptBusinessCard_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['ITER_CODE'] == '1':
                name_file = f'{NCRD.GetRandomNumber()}_AcceptBusinessCard.json'   
                form_data = AcceptBusinessCard(row['IDN_CARD'], name_file)
                cardcode, bon_code = form_data
                row['CARDCODE'] = cardcode
                row['BON_CODE'] = bon_code
                NCRD.UpdateDatasetTableDB('ndeacrd_data', data_list)
            elif row['IDN_CARD'] is None and row['DEPO_CODE'] == 'AA6':
                name_file = f'{NCRD.GetRandomNumber()}_SendCardAccept.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Отправка на акцептование карты {row['idn_card']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
            
def AcceptBusinessCard(idn_card, name_file):
