...
def BlockCardProduct_AllDataSet():
    """ Блокировка пластиковой карты клиента по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_BlockCardProduct.json'   
            BlockCardProduct(row['IDN_CARD'], name_file)

            
def BlockCardProduct(idn_card, name_file):
    """ Блокировка пластиковой карты клиента """
    NCRD = CardFramework() 
