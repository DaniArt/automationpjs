...
def JuniorATMDeposit_AllDataSet():
    ...
    for row in data_list:
      try:
          if row['IDN_CARD']:
              name_file = f'{card.GetRandomNumber()}_JuniorATMDeposit.json' 
              JuniorATMDeposit(row['VAL_CODE'], row['SUM_IN'], row['ACC_CRD'], name_file)
...
def JuniorATMDeposit(val, sum_in, acc_crd, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Junior карта", f"Пополнение счета через операцию 2405 по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
