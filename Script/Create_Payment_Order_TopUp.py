...
def CreatePaymentOrderTopUp_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['STEP_CASE'] =='pm_jur':
                name_file = f'{PaydocDataset.GetRandomNumber()}_CreateDocumentCredit.json'
                get_doc_number = CreatePaymentOrderTopUp(row['ORDER_TYPE_PAY'], row['OPERATION_CODE_PAY'], row['WND_NAME'], 
...
def CreatePaymentOrderTopUp(order_type, oper_code,main_wnd,payer_acc,client_IIN,recipient_acc,recipient_IIN,describe, KNP, KOD, KBE, no_commis, fast_pay, recipient_iin, val_code,cli_type,signature,name_file):
    doc_number = ''  
    Paydoc = Sordpay()
    ...
    try:
        ...
        save_but.Click()
        if Sys.Process("COLVIR").WaitDialog("Банковская система COLVIR", 3000).Exists:
          Sys.Process("COLVIR").Dialog("Банковская система COLVIR").Button("ОК").Click()
        Paydoc.CheckWarningPayerAccount()
        Paydoc.CheckWarningPayerAccount()
        # окно ввода СМС кода
        ...
        if not Sys.Process("COLVIR").WaitVCLObject("Контекст", 30000).Exists:
         operations_but.Click()
        ...
        Paydoc.SetSMSCode311()
        Delay(6000)
        if Sys.Process("COLVIR").WaitDialog("Подтверждение", 120000).Exists:
