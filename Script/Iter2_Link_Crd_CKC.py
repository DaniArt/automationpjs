...
def Iter2LinkCrdCKC_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{Ndeacrd.GetRandomNumber()}_Iter2LinkCrdCKC.json' 
            form_data = Iter2LinkCrdCKC(row['PRODUCT'], row['DOC_CODE'], row['REG_DATE'], name_file)
            row['IDN_CARD'] = form_data
            Ndeacrd.UpdateDatasetTableDB('ckcdoc_data', data_list)
             
def Iter2LinkCrdCKC(product, doc_code, reg_date, name_file):
