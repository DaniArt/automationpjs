...
def ReUndoRefusalAccept_AllDataSet():
    ...
    for row in data_list:
        if row['ITER_CODE'] == '2':
            name_file = f'{NCRD.GetRandomNumber()}_ReUndoRefusalAccept.json'   
            ReUndoRefusalAccept(row['IDN_CARD'], name_file)
            
def ReUndoRefusalAccept(idn_card, name_file):
    NCRD = CardFramework() 
