...
def UndoSendStopList_AllDataSet():
    """ Отмена постановки в стоп-лист """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    data_list = NCRD.ReadDatasetFromDB('ckcdoc_data')
    for row in data_list:
        if row['ITER_CODE'] == '1':
            name_file = f'{NCRD.GetRandomNumber()}_UndoSendStopList.json'   
            UndoSendStopList(row['IDN_CARD'], name_file)
            
def UndoSendStopList(idn_card, name_file):
    """ Отмена постановки в стоп-лист """
    NCRD = CardFramework() 
    ...
    value = ["Отмена постановки в стоп-лист", f"Отмена постановки в стоп-лист по {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:      
        ...
        NCRD.FindNeedOperation("Отмена постановки в стоп-лист") 
        NCRD.ConfirmOperation("Отмена постановки в стоп-лист")
        ...
        if status_crd[0].replace("\'", '') == 'Отменапостановкивстоп-лист':
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Отмена постановки в стоп-лист", "passed", {"message": f"Карта с идентификатором {status_crd[1]} отмена поставки в стоп лист"},
                                         'VCLObject("frmOperJournal")', "Выполнен", new_path, "passed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Checkpoint(f'Карта с идентификатором {status_crd[1]} отмена поставлени в стоп-лист')
        else:
            # Отчетность
            NCRD.AllureReportTemplate(abs_path, name_file, "Отмена постановки в стоп-лист", "failed", {"message": f"Карта с идентификатором {status_crd[1]} не отменена поставка"},
                                         'VCLObject("frmOperJournal")', "Ошибка", new_path, "failed", 1, 1, rm = True)
            # --------------------------------------------------------------------------------------------------------------------------
            Log.Warning(f'Карта с идентификатором {status_crd[1]} не отменена поставка')
