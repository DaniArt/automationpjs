...
def RepSendCardAccept_AllDataSet():
    """Повторная отправка на акцептование карты клиента по всему датасету """
    NCRD = CardFramework()
    NCRD.LoginInColvir()
    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['DEPO_CODE'] == 'AA6':
                name_file = f'{NCRD.GetRandomNumber()}_RepSendCardAccept.json'  
                ...
                name_file = f'{NCRD.GetRandomNumber()}_RepSendCardAccept.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Повторная отправка на акцептование карты", 
...
def SendCardAccept(idn_card,cl_code,name_file):
    ...
    value = ["Повторная отправка на акцептование карты после отмены отправки", f"Повторная отправка на акцептование карты {idn_card}", NCRD.GetDateTimeMilli()]
    ...
    try:   
      ...
      if status_crd[0].replace("\'", '') == 'Переданонаакцептование':
        # Отчетность
        NCRD.AllureReportTemplate(abs_path, name_file, "Повторная отправка на акцептование карты клиента", "passed", {"message": f"Карта с идентификатором {status_crd[1]} в состоянии {status_crd[0]}"},
        ...
        # Отчетность
        NCRD.AllureReportTemplate(abs_path, name_file, "Повторная отправка на акцептование карты клиента", "failed", {"message": f"Операция {status_crd[0]} не выполнена"},
