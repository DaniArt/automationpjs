...
def CreateArchiveInfo(idn_card, name_file):
    try:
        ...
        NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Операции - СКС Договора", "Отмена архивной выписки", name_file)
        #---------   
        NCRD.TaskInput('NCRD')
        ...
        # Выполнение операции
        NCRD.PerformingOperation('Отмена архивной выписки', 'Выполнить операцию "Отмена архивной выписки" ?', 'NCrdList', 'btnRunOperation')
        ...
        NCRD.CheckOperJrn("Отменаархивнойвыписки", "NCrdList", 'VCLObject("btnOperJrn")', "frmOperJournal", ["NAME"], count, name_file)
        count += 1
        #---------
