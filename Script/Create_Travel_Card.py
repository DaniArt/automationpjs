...
def CreateTravelCard_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['DOC_CODE']:
                name_file = f'{Ndeacrd.GetRandomNumber()}_CreateTravelCard.json' 
                form_data = CreateTravelCard(row['PRODUCT'], row['DOC_CODE'], row['DEPO_CODE'], row['TARIF_CODE'], name_file)
                row['IDN_CARD'] = form_data
                Ndeacrd.UpdateDatasetTableDB('travel_data', data_list)
            elif row['DOC_CODE'] is None:
                name_file = f'{Ndeacrd.GetRandomNumber()}_CreateTravelCard.json'
                Ndeacrd.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Подвязка договора СКС Travel", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
             
def CreateTravelCard(product, doc_code, depo, tarif, name_file):
