...
def TravelCommis2553_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD']:
                name_file = f'{card.GetRandomNumber()}_TravelCommis2553.json' 
                TravelCommis2553(row['VAL_CODE'], row['ACC_CRD'], row['SUM_TRN'], name_file)  
            elif row['IDN_CARD'] is None:
                name_file = f'{card.GetRandomNumber()}_TravelCommis2553.json'
                card.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Сервисная комиссия 2553 Travel карта клиента {row['CLI_ID']}", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')                                                                             
                                                                                                            
                                                                                                            
def TravelCommis2553(valcode, acc_crd, amount, name_file):
    soap = SoapRequests()
    task = TaskMethods()
    card = CardFramework()
    count = 1
    ...
    try:
        ...
        soap.CreateAllureReport("Colvir. Модуль - БОКС/ПС", "Продукт - Travel карта", f"Сервисная комиссия 2553 Travel карта по счету {acc_crd}", name_file)
        #---------    
        soap.StartCapJob()
