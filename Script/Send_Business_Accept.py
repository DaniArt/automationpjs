...
def SendBusinessAccept_AllDataSet():
    ...
    for row in data_list:
        try:
            if row['IDN_CARD'] and row['ITER_CODE'] == '1':
                name_file = f'{NCRD.GetRandomNumber()}_SendBusinessAccept.json'  
                form_data = SendBusinessAccept(row['IDN_CARD'], name_file)
                row['ACC_CRD'] = form_data
                NCRD.UpdateDatasetTableDB('ndeacrd_data', data_list)
            elif row['IDN_CARD'] is None and row['DEPO_CODE'] == 'AA6':
                name_file = f'{NCRD.GetRandomNumber()}_SendBusinessAccept.json'
                NCRD.CreateAllureReport("Colvir. Модуль - БОКС/ПС", f"Отправка на акцептование карты Юр лицо", 
                                                  f"{TestVariables().get_var('_dcmnt_nt_fnd')}", name_file)
        except Exception as e:
            Log.Event('Возникла ошибка идем дальше')
             
def SendBusinessAccept(idn_card, name_file):
